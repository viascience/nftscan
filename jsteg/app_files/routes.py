...
def index():
    ...
    if image is None:
        ...
        response = subprocess.run(
            ["jsteg", "reveal", image_path, f"/dataImages/{output_file}"],
            capture_output=True,
        )
        ...
        malware = (
            True if b"jpeg does not contain hidden data\n" != response.stderr else False
        )

        # Captures if jsteg cant find the path
        if malware == True and b"no such file or directory\\n" in response.stderr:
            response_dict = {"info": str(outcome)}
            return response_dict, 400

    response_dict = {"malware": malware, "info": str(outcome), "output": output_file}
